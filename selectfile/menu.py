...
class Variable:
    ...
    color_bleu_init = (10, 10, 250)
    color_bleu = (10, 10, 250)
    color_blanc_init = (250, 250, 250)
    color_blanc = (250, 250, 250)
    color_rouge_init = (250, 10, 10)
    color_rouge = (250, 10, 10)
    color_vert_init = (10, 250, 10)
    color_vert = (10, 250, 10)

    check_menu: dict[str, bool] = dict()
    ...
    @classmethod
    def get_menu(cls, menu: str) -> bool:
        return cls.check_menu.get(menu, False)

    @classmethod
    def toggle(cls, menu: str) -> None:
        cls.check_menu[menu] = not cls.check_menu.get(menu, False)
...
def select_color(color):
    Variable.selected_color = color
    fprint(color, "selected")    


def merge_color(fonction, color: str):
    if Variable.selected_color is None:
        return

    fprint("merging", color, "with", Variable.selected_color)
    selected_color = [0, 0, 0]
    for idx in range(3):
        selected_color[idx] = fonction(Variable.selected_color[idx], getattr(Variable, color)[idx])
    setattr(Variable, color, tuple(selected_color))
    fprint(getattr(Variable, color))
...
class Action:
        ...
        if rac:
            match rac:
                ...
                case rac if rac[0] == "<":
                    ...
                    Variable.set_menu(args[0], rac[1].lower() == "x")
                    self.check_value = Variable.get_menu(args[0])
                    self.raccourci = ""
    ...
    def toggle(self):
        fprint(">toggle", self.check_value)
        self.check_value = not self.check_value
        fprint("<toggle", self.check_value)
...
class Menu:
    ...
    opened_sub_menu: Optional[Menu] = None
    ...
    def compute(self):
        ...
        # Surface du check menu
        self.check_surf = pygame.Surface((self.goutiere_size-4, self.select_size))
        self.check_surf.fill((221, 236, 255))
    ...
    def draw(self, mouse_pos) -> None:
        ...
        if (self.border_size < mouse_pos[1] < self.surf.get_height()-self.border_size and 
            ...
            if self.selected_index > -1 and not self.liste_actions[self.selected_index].is_sub_menu():
                self.selected_index = -1
            self.index = -1
        ...
        if index > -1:
            select_position = self.get_position_from_index(index)
            match self.liste_actions[index].etat:
                case "actif":
        ...
        if self.index != self.old_index:
            ...
            if (self.index == -1 and self.old_index >= 0 and 
                    self.liste_actions[self.old_index].is_sub_menu() and
            ...
            if (self.liste_actions[self.index].is_sub_menu() and 
            ...
            if self.time_for_action == 1:
                ...
                if self.liste_actions[self.index].is_sub_menu():
                    ...
                    # Fermeture du sub_menu encore ouvert
                    self.close_sub_menu()
        ...
        for idx, action in enumerate(self.liste_actions):
            ...
            if action.check:
                if action.check_value:
                    self.surf.blit(self.check_surf, (2, dy - self.select_size))

                    check = self.fontsymbole.render(u"\u221A", True, couleur)
                    # Ajout du check
                    dy_calc = dy-(self.select_size+height) // 2
                    self.surf.blit(check, 
                        (self.goutiere_size//2 - check.get_size()[0]//2, 
                        dy_calc))
    ...
    def click(self) -> str:
        result: str = ""
        # fprint(">", f"{self.titre}({self.index})({self.selected_index})", self.liste_actions[self.index].etat, end="")
        if self.index < 0 and self.opened_sub_menu:
            # fprint(" *", end="")
...
def bloc_bleu(menu) -> None:
    ...
    sub_menu1.add(Action("Sous menu 16", "", "actif", fprint, "Sous menu 16"))
    sub_menu1.add(Action("Sous menu 17", "", "actif", fprint, "Sous menu 17"))
    sub_menu1.add(Action("", "", "separateur"))
    sub_menu1.add(Action("Sous menu 18", "", "actif", fprint, "Sous menu 18"))
    sub_menu1.add(Action("Sous menu 19", "", "actif", fprint, "Sous menu 19"))
    sub_menu1.add(Action("Sous menu 20", "", "actif", fprint, "Sous menu 20"))
    sub_menu1.add(Action("", "", "separateur"))
    ...
    menu.clear()
    menu.add(Action("Select Color", "", "actif", select_color, Variable.color_bleu))
    menu.add(Action("Merge Color", "", "actif", merge_color, max, "color_bleu"))
    menu.add(Action("Diff Color", "", "actif", merge_color, sub_avg, "color_bleu"))
    menu.add(Action("Avg Color", "", "actif", merge_color, add_avg, "color_bleu"))
    menu.add(Action("Invert Color", "", "actif", invert_color, "color_bleu"))
    ...
    menu.add(Action("Show signature", "<X", "actif", Variable.toggle, "show_signature"))
    menu.add(Action("", "", "separateur"))
    menu.add(Action("Show Diff Hunk", "<O", "actif", Variable.toggle, "show_diff_hunk"))
    menu.add(Action("Revert Diff Hunk", "<X", "actif", Variable.toggle, "revert_diff_hunk"))
    ...
    menu.add(Action("", "", "separateur"))
    menu.add(Action("Auto save on Build", "<x", "actif", Variable.toggle, "auto_save_on_build"))
...
def bloc_blanc(menu) -> None:
    ...
    menu.clear()
    menu.add(Action("Select Color", "", "actif", select_color, Variable.color_blanc))
    menu.add(Action("Merge Color", "", "actif", merge_color, max, "color_blanc"))
    menu.add(Action("Diff Color", "", "actif", merge_color, sub_avg, "color_blanc"))
    menu.add(Action("Avg Color", "", "actif", merge_color, add_avg, "color_blanc"))
    menu.add(Action("Invert Color", "", "actif", invert_color, "color_blanc"))
    menu.add(Action("", "", "separateur"))
...
def bloc_rouge(menu) -> None:
    ...
    menu.clear()
    menu.add(Action("Select Color", "", "actif", select_color, Variable.color_rouge))
    menu.add(Action("Merge Color", "", "actif", merge_color, max, "color_rouge"))
    menu.add(Action("Diff Color", "", "actif", merge_color, sub_avg, "color_rouge"))
    menu.add(Action("Avg Color", "", "actif", merge_color, add_avg, "color_rouge"))
    menu.add(Action("Invert Color", "", "actif", invert_color, "color_rouge"))
    menu.add(Action("", "", "separateur"))
...
def bloc_vert(menu) -> None:
    ...
    menu.clear()
    menu.add(Action("Select Color", "", "actif", select_color, Variable.color_vert))
    menu.add(Action("Merge Color", "", "actif", merge_color, max, "color_vert"))
    menu.add(Action("Diff Color", "", "actif", merge_color, sub_avg, "color_vert"))
    menu.add(Action("Avg Color", "", "actif", merge_color, add_avg, "color_vert"))
    menu.add(Action("Invert Color", "", "actif", invert_color, "color_vert"))
    menu.add(Action("", "", "separateur"))
...
def main() -> None:
    ...
    while Variable.Running:
        ...
        if Variable.Bleu:
            bleu = pygame.draw.rect(screen, Variable.color_bleu, (100, 200, 250, 400))
        if Variable.Blanc:
            blanc = pygame.draw.rect(screen, Variable.color_blanc, (400, 200, 250, 300))
        if Variable.Rouge:
            rouge = pygame.draw.rect(screen, Variable.color_rouge, (700, 200, 250, 300))
        if Variable.Vert:
            vert = pygame.draw.rect(screen, Variable.color_vert, (1000, 200, 300, 300))
