...
class Icone:
    ...
    def move(self, dx=0, dy=0):
        if dx or dy:
            ...
            self.dest_rect = pygame.Rect(posx, posy, Icone.ICONE_WIDTH, Icone.ICONE_HEIGHT)
...
class Window:
    ...
    def set_surface_color(self):
        self.top_surf.fill(self.theme_color())
        self.set_win_buttons_surface_color()
        if self.last_statut() != "MAXIMIZED":
            self.bottom_surf.fill(self.theme_color())
            self.left_surf.fill(self.theme_color())
            self.right_surf.fill(self.theme_color())
        self.title_surf.fill(self.theme_color())
    ...
    def theme_color(self, active_color=THEME_ACTIVE_COLOR, inactive_color=THEME_INACTIVE_COLOR):
...
class OperatingSystem:
    ...
    def __init__(self):
        self.running = True
        Audio.init(False)
        # self.screen = pygame.display.set_mode((1600, 800), pygame.FULLSCREEN, 24)
    ...
    def get_applications(self):
        ...
        def get_classes_from_file(fichier):
        ...
        def get_all_classes():
            ...
            for file in os.scandir("."):
                ...
                if file.is_file() and fichier.endswith(".py") and fichier != __file__[1+__file__.rindex(separateur):]:
                    fichier = fichier[:-3]
                    # print("\n[", fichier, "]")
                    for classe in get_classes_from_file(fichier):
                        # print(" ", classe, end=", ")
        ...
        for une_classe in get_all_classes():
    ...
    def open(self, title, couleur, app, *args):
        ...
        if (1+len(self.liste_taches))*(Tache.TASK_WIDTH+self.TASK_BAR_DECAL) > self.barre_taches_rect[2]-self.TASK_BAR_MENU_WIDTH:
            Tache.TASK_WIDTH = (self.barre_taches_rect[2]-self.TASK_BAR_MENU_WIDTH) // (1+len(self.liste_taches)) - self.TASK_BAR_DECAL
        self.resize_taches()
            ...
            if tache.window == fenetre:
                ...
                if Tache.TASK_WIDTH < Tache.TASK_WIDTH_MAX:
                    Tache.TASK_WIDTH = min(Tache.TASK_WIDTH_MAX, 
                        (self.barre_taches_rect[2]-self.TASK_BAR_MENU_WIDTH) // len(self.liste_taches) - self.TASK_BAR_DECAL)
                    self.resize_taches()
                else:
    ...
    def draw(self):
        # diminution de 25% de la puissance allouee aux fenetres non actives
        self.tick = 0 if (self.tick > 3) else self.tick+1
        ...
        for fenetre in self.liste_fenetres:
            if self.tick or fenetre.active:
                fenetre.update()
                   ...
                   fen_active.top_rect.collidepoint(mouse_position)):
                    if fen_active.close_rect.collidepoint(Mouse.get_saved_pos()) and (
                       fen_active.close_rect.collidepoint(mouse_position)):
                        ...
                        self.activate_last_window()
                        window_spotted = True
                    elif "RESIZABLE" in fen_active.properties and (
                         fen_active.max_rect.collidepoint(Mouse.get_saved_pos())) and (
    ...
    def mouse_move(self, mouse_position):
        ...
        if self.icone_catched_by_mouse:
            ...
            self.icone_catched_by_mouse.move(mouse_position[0]-Mouse.get_pos()[0], 
                mouse_position[1]-Mouse.get_pos()[1])
