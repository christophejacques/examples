    ...
    def draw(self):
        for i, etoile in enumerate(self.liste):
            ...
            if res:
                ...
                self.draw_line(couleur, pstar, star)
                self.draw_circle(couleur, star, taille)
            else:
                self.liste.pop(i)
...
class Lignes(AbstractZoneContent):
    ...
    def draw(self):
        for ligne in self.liste:
            self.draw_line(ligne.color, *ligne.get_coords())
...
class Graphical_Text(AbstractZoneContent):
    ...
    def __init__(self, texte, font="Arial", taille=12, couleur=(255, 255, 255),
        ...
        self.pfont = self.SysFont(font, taille)
    ...
    def set_texte(self, texte: str, num_ligne: int = 1) -> None:
        if len(self.texte) >= num_ligne:
            self.texte[num_ligne-1] = self.pfont.render(texte, 0, self.couleur)
        else:
