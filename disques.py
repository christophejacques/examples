	...
	def dir(self, directory : str = "."):
		...
		if self.nb_dirs() > 0:
			...
			for i, rep in enumerate(self.get_dirs()):
				nom_rep = ("{:" + str(taille) + "s} ").format("[{}]".format(rep.name))
		...
		if self.nb_files() > 0:
			...
			for i, fichier in enumerate(self.get_files()):
				nom_fichier = ("{:" + str(taille) + "s}").format(fichier.name) + "{:>12s}    ".format(Disk.int2human(fichier.stat().st_size))
		...
		print()
		print(f"\nTotal Reps: {self.nb_dirs()}, Fichiers: {self.nb_files()}, Taille Fichiers:", Disk.int2human(self.get_size()))
