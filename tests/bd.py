...
class Table:
    ...
    def setSchema(self, schema: str) -> None:
        ...
        for champ in self.champs:
            champ_table = champ.getTable()
            if "." not in champ_table:
                champ.setTable(f"{schema}.{champ_table}")
...
class BaseDeDonnees:
    ...
    def getSchemas(self) -> list:
        return [schema for schema in self.tables]
    ...
    def add_table(self, table: Table) -> None:
        ...
        if table.getSchema() == "":
            table.setSchema(self.current_schema)
    ...
    def drop_table(self, table: Table) -> None:
        ...
        for schema in self.getSchemas():
            for other_table in filter(lambda vtable: vtable != table, self.tables[schema]):
...
with BaseDeDonnees() as bd:
    ...
    print("\n>", bd.getChampByFullName("users.nom"))

    # bd.dropTableByName("ref.fonctions")
